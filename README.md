...
**Hyprland 0.55+ with Lua config** is required. `hyprtoggle` uses `hyprctl eval` and `hyprctl dispatch` with the Lua dispatcher API (`hl.dsp.*`) introduced in Hyprland 0.55. It is not compatible with the legacy hyprlang `.conf` config format.
